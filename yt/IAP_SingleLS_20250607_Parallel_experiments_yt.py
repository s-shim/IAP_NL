...
def LS(bestTotalRevenue,bestRevenue,bestIs_offered,bestOptionsOffered,bestTpw,confG,pw,logSum,columnPackage,inputPackage):
    ...
    result.to_csv(r'parallel_single2/result_LS/result_LS_logSum%s_Network%s_Rep%s_SingleQ%s.csv'%(int(logSum*100),networkID,rep,singleQ), index = False)#Check
...
## Instances
for (networkID,repNum) in [('yt',1)]:#[(0,50),(1,10),(2,50),(3,50),(4,50),(5,50),(6,10),(7,10),(8,50),(9,50)]:
    ...
    for rep in range(repNum):            
        ...
        if __name__ == '__main__':
            ...
            loptSolution.to_csv(r'parallel_single2/lopt/lopt_LS_Single%s_Net%s_Rep%s_%s_logSum%s.csv'%(bestSingleQ,networkID,rep,machineName,int(logSum*100)), index = False)#Check
            ...
            grandTable.to_csv(r'parallel_single2/summary/Summary_LS_Single_%s_%s_logSum%s.csv'%(networkID,machineName,int(logSum*100)), index = False)#Check
